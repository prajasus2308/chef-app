...
export default function App() {
  ...
  const generateRecipe = async (imageB64?: string) => {
    ...
    try {
      ...
      const parts: any[] = imageB64 
        ? [{ inlineData: { mimeType: 'image/jpeg', data: imageB64.split(',')[1] } }, { text: `Gourmet recipe for items in image. ${prompt}` }]
        : [{ text: `Gourmet recipe for: ${prompt}` }];
      ...
      setCurrentRecipe({ ...data, id: crypto.randomUUID(), createdAt: Date.now() });
      setCapturedImage(null);
    } catch (error) {
  ...
  const addManualDish = () => {
    ...
    const newRecipe: Recipe = {
      ...
      description: customDish.desc || "A classic homemade dish.",
  ...
  const visualizeRecipe = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: { parts: [{ text: `Gourmet food photograph of ${currentRecipe.title}.` }] },
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 font-sans">
      ...
      <nav className="w-full md:w-20 lg:w-64 bg-white border-r border-slate-200 p-4 flex md:flex-col gap-2 z-10 sticky top-0 md:h-screen shadow-sm">
        <div className="hidden md:flex items-center gap-3 px-4 py-6 mb-4">
          ...
          <span className="font-bold text-xl lg:block hidden tracking-tight">ChefGenius</span>
        </div>
        ...
        {view === 'kitchen' && (
          <div className="animate-fade-in space-y-10">
            <header className="space-y-2">
              ...
              <p className="text-slate-500">Prompt your craving or scan your ingredients.</p>
            </header>
        ...
        {view === 'pantry' && (
          <div className="animate-fade-in space-y-10">
            <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6">
              <div className="space-y-2">
                <h1 className="text-4xl font-extrabold text-slate-900">Your Pantry</h1>
        ...
        {view === 'shopping' && (
          <div className="animate-fade-in space-y-10 max-w-2xl mx-auto">
            <header className="space-y-2">
              ...
              <p className="text-slate-500">Groceries and recipe essentials.</p>
            </header>
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 animate-fade-in">
          <div className="bg-white w-full max-w-lg rounded-[2rem] shadow-2xl overflow-hidden flex flex-col border border-white">
            ...
            <div className="p-8 space-y-6">
              ...
              <button onClick={addManualDish} disabled={!customDish.title} className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold shadow-xl hover:bg-slate-800 transition-all disabled:opacity-30">Save to Pantry</button>
            </div>
          </div>
        </div>
      )}

      {/* Other Modals (Camera, Cooking Mode) - Re-used for brevity */}
      {isCameraOpen && (
        <div className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center p-6">
          ...
          <div className="mt-12 flex gap-6">
            {!capturedImage ? <button onClick={capturePhoto} className="w-20 h-20 bg-white rounded-full border-4 border-slate-400" /> : <>
              ...
              <button onClick={() => generateRecipe(capturedImage!)} className="bg-orange-600 text-white px-8 py-4 rounded-2xl font-bold">Analyze Photo</button>
            </>}
          </div>
        </div>
      )}
      ...
      {cookingMode && (
        <div className="fixed inset-0 z-[110] bg-white flex flex-col animate-fade-in overflow-y-auto">
          <header className="p-6 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white/80 backdrop-blur">
            ...
            <div className="text-center"><h2 className="font-bold text-slate-900">{cookingMode.title}</h2><p className="text-xs text-slate-400">Step {currentStep + 1} of {cookingMode.instructions.length}</p></div>
            <div className="w-16" />
          </header>
